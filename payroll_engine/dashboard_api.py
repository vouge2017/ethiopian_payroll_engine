...
def get_dashboard_data(user, company_id, db, models, role_filter=None):
    ...
    ).order_by(PayrollRun.run_date.desc()).limit(4).all()
...
def _owner_metrics(current_run, recent_runs, company_id, db, models):
    ...
    total_pension = float(sum(ps.employee_pension or 0 for ps in payslips) +
                          sum(ps.employer_pension or 0 for ps in payslips))
...
def _employee_metrics(user, company_id, db, models):
    ...
    emp = Employee.query.filter_by(
        company_id=company_id, user_id=user.id, is_deleted=False
    ).first()
    ...
    # Latest payslip
    latest_payslip = Payslip.query.join(PayrollRun).filter(
    ...
    balances = LeaveBalance.query.filter_by(
        company_id=company_id, employee_id=emp.id, year=date.today().year
    ).all()
