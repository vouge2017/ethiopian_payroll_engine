...
def test_register_phone_input_uses_intl_tel(app):
    """Register must use the phone-prefix-box wrapper for phone input."""
...
def test_phone_wrapper_css_handles_intl_tel(app):
    """The .onboarding-phone-wrapper CSS must include rules for the
...
def test_static_251_prefix_is_removed_from_register(app):
    ...
    has been removed from the register page — the new phone-prefix-box
    spans are generated by phone-input.js. This test just verifies the
    old pattern is gone.
    """
    client = app.test_client()
    ...
    assert 'class="onboarding-phone-prefix">+251' not in html, (
        'Register page still has the old hardcoded +251 prefix span'
    )


def test_login_has_phone_tabs_for_phone_or_email(app):
    """Login's `login_id` is phone-or-email. The tab switching is
    present so users can choose phone or email mode.
    """
    client = app.test_client()
...
def test_forgot_password_has_no_static_251_prefix(app):
    """Same as login — forgot_password's `login_id` uses tabs."""
    client = app.test_client()
...
def test_emergency_phone_uses_phone_prefix(app):
    """The employee-portal emergency_phone field must use the phone
    prefix box so employees can register a phone number."""
    client = app.test_client()
    ...
    html = r.get_data(as_text=True)
    # emergency_phone must have the phone prefix box
    assert 'phone-prefix-box' in html
    assert '+251' in html
...
def test_phone_prefix_box_height_matches_input(app):
    ...
    ppb_block = css.split('.phone-prefix-box {', 1)
    assert len(ppb_block) == 2, '.phone-prefix-box block not found'


def test_phone_input_with_prefix_height_matches(app):
    """The input next to the prefix box must also be 44px tall."""
    css = app.test_client().get('/static/css/design-system.css').get_data(as_text=True)
    pip_block = css.split('.phone-input-with-prefix input', 1)
    assert len(pip_block) == 2, '.phone-input-with-prefix input block not found'
    body = pip_block[1][:800]
    assert 'height: 44px' in body


def test_phone_prefix_focus_has_brand_color(app):
    """The input next to the prefix box must use brand color on focus."""
    css = app.test_client().get('/static/css/design-system.css').get_data(as_text=True)
    focus_block = css.split('.phone-input-with-prefix input[type="tel"]:focus', 1)
    assert len(focus_block) == 2, '.phone-input-with-prefix input:focus block not found'
