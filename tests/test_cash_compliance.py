...
Ethiopian law (Proclamation No. 1395/2025) requires electronic payment
for salaries above ETB 30,000. The system flags this as a FLAG (not BLOCK)
— it informs the owner but does not prevent payroll from proceeding.
"""
import sys
import os
...
def _make_emp(name='Dawit Mekonnen', net=35000, bank=''):
    ...
    return {
        ...
        'taxable': 32900,
        'tax': 7500,
        'pension_employee': 2100,
        'pension_employer': 3300,
        'net': net,
        'bank': bank,
        'tin': '',
    }
...
def test_cash_compliance_flag_when_over_limit_no_bank():
    """Net pay > 30,000 AND no bank account → FLAG."""
    employees = [_make_emp(net=35000, bank='')]
    ...
    assert '35,000' in cash_flags[0].message
    assert '30,000' in cash_flags[0].message
...
def test_cash_compliance_ok_when_has_bank():
    """Net pay > 30,000 BUT has bank account → no flag."""
    employees = [_make_emp(net=35000, bank='telebirr:0911234567')]
    results = validate_payroll_data(employees)
    cash_flags = [r for r in results if r.rule_code == 'CASH_COMPLIANCE']
...
def test_cash_compliance_ok_when_under_limit():
    """Net pay < 30,000 AND no bank → no flag (under limit)."""
    employees = [_make_emp(net=25000, bank='')]
    results = validate_payroll_data(employees)
    cash_flags = [r for r in results if r.rule_code == 'CASH_COMPLIANCE']
    assert len(cash_flags) == 0
...
def test_cash_compliance_exactly_at_limit():
    """Net pay == 30,000 → no flag (at limit, not above)."""
    employees = [_make_emp(net=30000, bank='')]
    results = validate_payroll_data(employees)
    cash_flags = [r for r in results if r.rule_code == 'CASH_COMPLIANCE']
    assert len(cash_flags) == 0
...
def test_cash_compliance_one_above_limit():
    """Net pay == 30,001 → FLAG."""
    employees = [_make_emp(net=30001, bank='')]
    results = validate_payroll_data(employees)
    cash_flags = [r for r in results if r.rule_code == 'CASH_COMPLIANCE']
    assert len(cash_flags) == 1
    assert cash_flags[0].severity == 'FLAG'
...
def test_cash_compliance_multiple_employees():
    ...
    employees = [
        _make_emp(name='Alemayehu', net=35000, bank=''),           # FLAG
        _make_emp(name='Tigist', net=40000, bank='cbe:1000123'),   # OK
        _make_emp(name='Hana', net=20000, bank=''),                # OK (under limit)
    ]
