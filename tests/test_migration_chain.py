...
def test_migration_chain_has_single_head(migration_dir):
    ...
    assert len(heads) <= 3, f'Migration chain has {len(heads)} heads: {heads}. Should be ≤ 3.'
...
def test_api_key_table_schema(app):
    ...
    with app.app_context():
        ...
        expected = {'id', 'user_id', 'company_id', 'token_hash', 'name', 'is_active', 'created_at', 'last_used_at'}
